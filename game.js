...
function onNewPlayer(data) {
	...
	if (players.length === 2) {
	    util.log('create new board');
		tttGame = new tttBoard.Board(players[0].id, players[1].id);
	}
}

// Player has moved
function onMovePlayer(data) {
    util.log("Current boardstate is: "+tttGame.getGameState());
	...
	tttGame.setTurn(movePlayer.id, data.index);

	// Broadcast updated position to connected socket clients
	//this.broadcast.emit("gameState", tttGame.getGameState());
	socket.emit("gameState", tttGame.getGameState());
}
